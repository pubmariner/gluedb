...
report_csv =CSV.open("/Users/Varun/Desktop/reports/oct_18/premium_report_glue.csv", "w")
report_csv << %w(family.e_case_id policy.eg_id  policy.plan.name  policy.renewal_plan.name pre_amt_tot hbx_enrollment.policy.policy_start  policy.aasm_state policy.plan.coverage_type policy.plan.metal_level person.authority_member_id  person.authority_member.is_incarcerated person.authority_member.citizen_status  person.authority_member.is_state_resident is_dependent  is_responsible_party?)
begin
  ...
  @data.each do |d|
    ...
    report_csv.add_row([d["family.e_case_id"],d["policy.eg_id"],d["policy.plan.name"],d["policy.renewal_plan.name"],[renewal_policy.pre_amt_tot.to_s],d["hbx_enrollment.policy.policy_start"],d["policy.aasm_state"],d["policy.plan.coverage_type"],d["policy.plan.metal_level"],d["person.authority_member_id"],d["person.authority_member.is_incarcerated"],d["person.authority_member.citizen_status"],d["person.authority_member.is_state_resident"],d["is_dependent"],d["is_responsible_party?"]])
  end  
rescue Exception => e
