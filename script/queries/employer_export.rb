...
Employer.all.each do |employer|
  ...
  employer.plan_years.each do |plan_year|
    row = [employer.name, employer.hbx_id, employer.name_first, employer.name_middle, employer.name_last, employer.fein, employer.addresses.last.address_1, employer.addresses.last.address_2, employer.addresses.last.city, employer.addresses.last.state, employer.addresses.last.zip, employer.phones.last.phone_number, employer.emails.try(:last).try(:email_address)]
