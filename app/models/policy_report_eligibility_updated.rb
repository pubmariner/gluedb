...
class PolicyReportEligibilityUpdated
  ...
  index({event_time: 1, event_name: 1, policy_id: 1, eg_id: 1})

  def self.newest_event?(new_policy_id, new_eg_id, new_event_name, new_event_time)
    !self.where(:policy_id => new_policy_id, :event_name => new_event_name, :event_time => {"$gte" => new_event_time}).any?
  end
  ...
  def self.create_new_event_and_remove_old(new_policy_id, new_eg_id, new_event_name, new_event_time, new_payload, match_criteria)
  ...
  def self.store_and_yield_deleted(new_policy_id, new_eg_id, new_event_name, new_event_time, new_payload)
    if not_yet_seen_by_carrier?(new_policy_id, new_eg_id)
      ...
      create_new_event_and_remove_old(
        ...
        new_event_time,
        new_payload,
        {:policy_id => new_policy_id, :eg_id => new_eg_id, :event_name => new_event_name}) do |old_record|
          yield old_record
      end
    end
  end
