...
class UpdateTotalResponsibleAmount < MongoidMigrationTask
  ...
  def update_premium_amounts(eg_id, tot_res_amt, pre_amt_tot, tot_emp_res_amt)
    ...
    if policy.present? && policy.aptc_credits.empty?
      ...
      policy.save!
      puts "Successfully updated premium amounts eg_id:#{policy.eg_id}, tot_res_amt:#{policy.tot_res_amt.to_i}, pre_amt_tot:#{policy.pre_amt_tot.to_i}, tot_emp_res_amt:#{policy.tot_emp_res_amt.to_i}" unless Rails.env.test?
    else
