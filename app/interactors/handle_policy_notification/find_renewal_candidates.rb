module HandlePolicyNotification
  class FindRenewalCandidates
    ...
    # - carrier_switch_renewals (a set of carrier switch renewals)
    def call
      possible_renewals = []
      carrier_switch_renewals = []
      ...
      if context.policy_details.market != "shop"
        if (coverage_start.day == 1) && (coverage_start.month == 1)
           ...
           carrier_switch_renewals = subscriber_person.policies.select do |pol|
             is_an_ivl_carrier_switch_renewal_compared_to(pol, context.plan_details.found_plan, coverage_start)
           end
        end
      end
    ...
    def is_an_ivl_carrier_switch_renewal_compared_to(pol, plan, coverage_start)
      ...
      (plan.coverage_type == pol.plan.coverage_type)
    end
