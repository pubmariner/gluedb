...
describe MergeDuplicatePeople, dbclean: :after_each do
  ...
  describe 'merge the people' do 
    ...
    before(:each) do 
      allow(ENV).to receive(:[]).with("person_to_keep").and_return(person_to_keep.authority_member_id)
      allow(ENV).to receive(:[]).with("person_to_remove").and_return(person_to_remove.authority_member_id)
    end
