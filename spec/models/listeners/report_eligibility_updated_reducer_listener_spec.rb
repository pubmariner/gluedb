...
describe Listeners::ReportEligibilityUpdatedReducerListener do
  ...
  let(:mock_new_report_eligibility_updated_reducer_listener) do
    Listeners::ReportEligibilityUpdatedReducerListener.new(mock_channel, mock_create_queues)
  end
  ...
  subject { Listeners::ReportEligibilityUpdatedReducerListener }
  ...
  before :each do
    ...
    allow(subject).to receive(:new).with(mock_channel, mock_create_queues).and_return(mock_new_report_eligibility_updated_reducer_listener)
    allow(mock_new_report_eligibility_updated_reducer_listener).to receive(:subscribe).with(:block => true, :manual_ack => true, :ack => true)
    allow(mock_connection).to receive(:close).and_return(mock_close)
    allow(mock_new_report_eligibility_updated_reducer_listener).to receive(:passes_validation?).with(
      true, true, true
    ).and_return(true)
    # Existing try count to trigger on_message(delivery_info, properties, payload)
    allow(mock_new_report_eligibility_updated_reducer_listener).to receive(:extract_retry_count).with(mock_p_headers).and_return(9)
  ...
  it "should trigger the policy events report eligibility updated on message" do
    ...
    mock_new_report_eligibility_updated_reducer_listener.on_message(mock_delivery_info, mock_properties, mock_body)
    old_record.reload
    expect(old_record_original_event_time).not_to eq(old_record.event_time)
  end
end
