...
describe Generators::Reports::IrsYearlySerializer, :dbclean => :after_each do
  ...
  before(:each) do
    ...
    plan.premium_tables.create!(rate_start_date: policy.coverage_period.first,rate_end_date: policy.coverage_period.last, age: ((policy.coverage_period.first.year - 1) -  Person.first.authority_member.dob.year), amount:12)
