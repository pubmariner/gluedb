...
describe EmployerEvents::Renderer, "given an xml, from which it selects carrier plan years", :dbclean => :after_each do
  ...
  let!(:plan_year) { instance_double(PlanYear, issuer_ids: [carrier.id], start_date: Date.strptime(start_date, "%Y%m%d"), end_date:Date.strptime(end_date, "%Y%m%d"), employer: employer)}
